...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const savedData = localStorage.getItem('TheWorkMind_client_data');
    if (savedData && !clientData) setClientDataState(JSON.parse(savedData));

    const savedConvos = localStorage.getItem('TheWorkMind_conversations');
    if (savedConvos) setConversations(JSON.parse(savedConvos));

    const savedDocs = localStorage.getItem('TheWorkMind_documents');
    if (savedDocs) setDepartmentDocuments(JSON.parse(savedDocs));

    const savedHubs = localStorage.getItem('TheWorkMind_hubs');
    if (savedHubs) setDepartmentHubs(JSON.parse(savedHubs));

    const savedDept = localStorage.getItem('TheWorkMind_active_department');
      ...
      try {
        ...
        if (status.completed && status.formId) {
          ...
          if (formRes.ok) {
            ...
            localStorage.setItem('TheWorkMind_client_data', JSON.stringify(mappedData));
  ...
  useEffect(() => {
    if (activeDepartment) {
      localStorage.setItem('TheWorkMind_active_department', activeDepartment);
    }
  }, [activeDepartment]);

  const setClientData = (data: IntakeData) => {
    setClientDataState(data);
    localStorage.setItem('TheWorkMind_client_data', JSON.stringify(data));
  };
  ...
  const addMessage = (dept: Department, role: 'user' | 'assistant', content: string, escalation?: any) => {
    setConversations(prev => {
      ...
      localStorage.setItem('TheWorkMind_conversations', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const addDocument = (dept: Department, doc: StoredDocument) => {
    setDepartmentDocuments(prev => {
      ...
      localStorage.setItem('TheWorkMind_documents', JSON.stringify(updated));
      return updated;
    });
  };
  ...
  const addHubMessage = (dept: Department, msg: HubMessage) => {
    setDepartmentHubs(prev => {
      ...
      localStorage.setItem('TheWorkMind_hubs', JSON.stringify(updated));
      return updated;
    });
  };

  const resetApp = () => {
    localStorage.removeItem('TheWorkMind_client_data');
    localStorage.removeItem('TheWorkMind_conversations');
    localStorage.removeItem('TheWorkMind_documents');
    localStorage.removeItem('TheWorkMind_hubs');
